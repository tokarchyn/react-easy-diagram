...
var generateState=function generateState(colNum,rowNum){var nodes=[];var links=[];var getNodeId=function getNodeId(i,j){return"node_"+i+"_"+j;};for(var i=0;i<colNum;i++){for(var j=0;j<rowNum;j++){nodes.push({id:getNodeId(i,j),position:[i*120,j*120],componentType:'star'});if(i-1>=0){links.push({source:{nodeId:getNodeId(i-1,j),portId:'right'},target:{nodeId:getNodeId(i,j),portId:'left'}});}if(j-1>=0){links.push({source:{nodeId:getNodeId(i,j-1),portId:'bottom'},target:{nodeId:getNodeId(i,j),portId:'top'}});}}}return{nodes:nodes,links:links};};/* harmony default export */ var examples_performanceDiagram = (function(){return/*#__PURE__*/react.createElement(dist.Diagram,{initState:generateState(10,10)});});
