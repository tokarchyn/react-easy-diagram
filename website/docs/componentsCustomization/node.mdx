  ...
  settings={{
    nodes: {
      components: {
        ...
        default: MyDefaultNode, // to override default node component, which is just a component with a label
      },
    },
  }}
  initState={{
...
1. `createInputOutputHorizontalNode(settings)` - create a component with input on the left border of the node and output on the right ('input' and 'output' means nothing but the `id` of port).
...
You can provide `base` classes and styles for node in [settings](#settings), which will be applied regardless of rather node is hovered or selected. You can also provide classes and styles that will be added to `base` ones when the node will be hovered, selected or both.
```jsx
createStarNode({
  classes: {
    base: ['my_node_base'],
    hovered: ['my_node_hovered'], // if node will be hovered the classes 'my_node_base' and 'my_node_hovered' will be added to component
...
Creating your own component for node is very simple and require only two things:
1. Wrapping in `observer` as were written at the beginning of this page.
2. Attaching draggable reference which component will get as a prop.

Let's write a node component with background depending on rather node is hovered or not. It will also display it's position and label. To see other data available in `entity` object, which you will also get as a prop, see [State management](../stateManagement#node).
```jsx
const NodeComponent = observer(({ entity, draggableRef }) => {
  return (
    <div
      ref={draggableRef} // this will enable library to track when user hover, drag the node etc. 
      style={{
        padding: 15,
