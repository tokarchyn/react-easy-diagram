...
To create built-in port component call `createPortInnerDefault(settings)`. You can provide classes and styles for port in settings for each state, like hovered, dragging or invalid. The `base` classes and styles are applied regardless of port state.
